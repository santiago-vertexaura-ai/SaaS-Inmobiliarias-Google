    ...
    <div className="flex h-screen w-full bg-[#F3F4F6] overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto relative flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {(!['dashboard', 'leads', 'integrations', 'whatsapp', 'inmuebles', 'settings', 'help'].includes(currentView)) && (
             <div className="flex flex-col items-center justify-center h-full text-neutral-400">
               <Plug size={48} className="mb-4 opacity-20" />
               <p>Esta sección está en construcción para el MVP.</p>
             </div>
          )}
          {/* Specific placeholder for known but unimplemented views */}
           {['inmuebles', 'settings', 'help'].includes(currentView) && (
             <div className="flex flex-col items-center justify-center h-full text-neutral-400">
               <Plug size={48} className="mb-4 opacity-20" />
               <p className="capitalize">Sección {currentView} próximamente.</p>
             </div>
          )}
        </div>
      </main>
