...
const EvolutionChart = () => {
  ...
  const max = 100; // Fixed max scale for better visualization

  return (
    <div className="w-full h-64 flex items-end justify-between gap-4 px-2 pb-2">
      {data.map((d, i) => (
        <div key={i} className="flex flex-col items-center gap-2 flex-1 h-full justify-end group cursor-pointer">
           <div className="relative w-full max-w-[40px] bg-neutral-100 rounded-t-sm h-full flex items-end overflow-hidden">
             <div 
               className="w-full bg-neutral-900 rounded-t-sm transition-all duration-500 group-hover:bg-neutral-800"
             ...
             <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-full opacity-0 group-hover:opacity-100 transition-opacity mb-2">
                <div className="bg-neutral-900 text-white text-[10px] py-1 px-2 rounded shadow-lg whitespace-nowrap">
                   {d.val} leads
                </div>
             </div>
           </div>
           <span className="text-xs font-medium text-neutral-400 group-hover:text-neutral-900 transition-colors">{d.day}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white p-6 rounded-xl border border-neutral-200 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-semibold text-neutral-900">Evolución de Leads</h3>
